...
if (process.platform !== "darwin") {
  console.log("Skipping Swift review eligibility contract outside macOS");
  process.exit(0);
}

const output = path.join(os.tmpdir(), `trace-review-eligibility-${process.pid}`);
const moduleCache = path.join(
  os.tmpdir(),
  `trace-review-eligibility-module-cache-${process.pid}`,
);
let exitStatus = 1;
try {
  const compile = spawnSync(
    ...
    [
      "swiftc",
      "iOS (App)/TraceReviewEligibility.swift",
      "test/swift/TraceReviewEligibilityContract.swift",
      "-module-cache-path",
      moduleCache,
      "-o",
      output,

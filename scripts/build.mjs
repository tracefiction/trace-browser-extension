...
const IOS_ONBOARDING_EXPERIMENT =
  process.env.TRACE_IOS_ONBOARDING_EXPERIMENT === "1";
if (IOS_ONBOARDING_EXPERIMENT && !IOS_PERMISSION_SPIKE) {
  throw new Error(
    "TRACE_IOS_ONBOARDING_EXPERIMENT requires TRACE_IOS_PERMISSION_SPIKE.",
  );
}
...
const EXPERIMENT_TRACE_API_BASE =
  "https://ff-app-development.up.railway.app";
const EXPERIMENT_TRACE_WEB_ORIGIN =
...
if (IS_RELEASE) {
  assertReleaseUrl(
    "TRACE_API_BASE",
    TRACE_API_BASE,
    IOS_ONBOARDING_EXPERIMENT
      ? EXPERIMENT_TRACE_API_BASE
...
const permissionSpikeBackground = path.join(
  RES,
  "permission-spike-background.js",
);
...
const popupConfig = [
  ...
  ...(IOS_ONBOARDING_EXPERIMENT
    ...
    : []),
  ...(HAS_SESSION_RUNTIME
    ? [`globalThis.TRACE_SESSION_MODE = ${JSON.stringify(SESSION_MODE)};`]
    : []),
].join("\n") + "\n";
...
`;
  ...
  packagedBackground = bg;
}
if (IOS_PERMISSION_SPIKE) {
  await esbuild({
    entryPoints: [path.join(ROOT, "src", "permission-spike-background.mjs")],
    outfile: permissionSpikeBackground,
    bundle: true,
    format: "iife",
    platform: "browser",
    target: ["safari17"],
  ...
  fs.writeFileSync(
    outBg,
    `${packagedBackground}\nglobalThis.TRACE_PERMISSION_WEB_ORIGIN = ${JSON.stringify(TRACE_WEB_ORIGIN)};\nimportScripts("permission-spike-background.js");\n`,
    "utf8",
  );
} else {
  fs.rmSync(permissionSpikeBackground, { force: true });
  fs.writeFileSync(outBg, packagedBackground, "utf8");
}
...
fs.writeFileSync(path.join(distChrome, "background.js"), packagedBackground, "utf8");
fs.writeFileSync(
...
fs.writeFileSync(path.join(distFf, "background.js"), packagedBackground, "utf8");
...
enum TraceWebOriginGenerated {
    ...
    static let allowReleaseExperimentOrigin: Bool = ${IOS_ONBOARDING_EXPERIMENT ? "true" : "false"}
}
`;
...
console.log(
  "TRACE_IOS_ONBOARDING_EXPERIMENT=" +
    (IOS_ONBOARDING_EXPERIMENT ? "1" : "0"),
);

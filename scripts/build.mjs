...
const IOS_POPUP_PERMISSION_SPIKE = /^(?:1|true)$/i.test(
  String(env.TRACE_IOS_POPUP_PERMISSION_SPIKE ?? ""),
);
...
if (IOS_POPUP_PERMISSION_SPIKE && SESSION_MODE !== "kernel") {
  throw new Error("TRACE_IOS_POPUP_PERMISSION_SPIKE requires TRACE_SESSION_MODE=kernel.");
}
...
const IOS_POPUP_PERMISSION_SPIKE_PERMISSIONS = ["scripting"];
const IOS_POPUP_PERMISSION_SCRIPT_IDS = Object.freeze({
  archive: "trace-popup-permission-archive-runtime",
  savedFilters: "trace-popup-permission-ao3-saved-filters",
});
...
manifest.permissions = unique([
  ...(manifest.permissions || []).filter(
    (permission) => !IOS_POPUP_PERMISSION_SPIKE_PERMISSIONS.includes(permission),
  ),
  ...SAFARI_ONLY_PERMISSIONS,
  ...(IOS_POPUP_PERMISSION_SPIKE ? IOS_POPUP_PERMISSION_SPIKE_PERMISSIONS : []),
]);
...
manifest.host_permissions = IOS_POPUP_PERMISSION_SPIKE
  ? safariHostPermissions.filter(
      (permission) => !SITE_HOST_MATCHES.includes(permission),
    )
  : safariHostPermissions;
if (IOS_POPUP_PERMISSION_SPIKE) {
  manifest.optional_host_permissions = [...SITE_HOST_MATCHES];
} else {
...
const popupPermissionContentScripts = [
  {
    id: IOS_POPUP_PERMISSION_SCRIPT_IDS.archive,
    matches: [...archiveContentScript.matches],
    js: [...archiveContentScript.js],
    excludeMatches: [...archiveContentScript.exclude_matches],
    persistAcrossSessions: true,
    runAt: archiveContentScript.run_at,
  },
  {
    id: IOS_POPUP_PERMISSION_SCRIPT_IDS.savedFilters,
    matches: [...savedFiltersContentScript.matches],
    js: [...savedFiltersContentScript.js],
    excludeMatches: [...savedFiltersContentScript.exclude_matches],
    persistAcrossSessions: true,
    runAt: savedFiltersContentScript.run_at,
  },
];
...
${HAS_SESSION_RUNTIME ? `globalThis.TRACE_SESSION_MODE = ${JSON.stringify(SESSION_MODE)};\n` : ""}${IOS_POPUP_PERMISSION_SPIKE ? `globalThis.TRACE_IOS_POPUP_PERMISSION_SPIKE = true;\nglobalThis.TRACE_ARCHIVE_PERMISSION_BUNDLE = ${JSON.stringify(SITE_HOST_MATCHES)};\nglobalThis.TRACE_ARCHIVE_PERMISSION_CONTENT_SCRIPTS = ${JSON.stringify(popupPermissionContentScripts)};\n` : ""}`;
const packagedPopupConfig = `// Generated by npm run build (scripts/build.mjs). Do not edit by hand.
globalThis.TRACE_EXTENSION_WEB_ORIGIN = ${JSON.stringify(TRACE_WEB_ORIGIN)};
${HAS_SESSION_RUNTIME ? `globalThis.TRACE_SESSION_MODE = ${JSON.stringify(SESSION_MODE)};\n` : ""}`;
...
const packagedSourceManifest = IOS_POPUP_PERMISSION_SPIKE
  ? {
      ...manifest,
...
console.log("TRACE_IOS_POPUP_PERMISSION_SPIKE=" + IOS_POPUP_PERMISSION_SPIKE);

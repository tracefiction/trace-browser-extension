...
const env =
  BUILD_MODE === "release" || BUILD_MODE === "shadow"
    ? { ...fileEnv, ...process.env }
    : { ...process.env, ...fileEnv };
const IS_RELEASE = BUILD_MODE === "release";
const IS_SHADOW = BUILD_MODE === "shadow";
...
if (!IS_SHADOW) syncXcodeMarketingVersion(version);
...
let chromeManifest = packagedManifest;
if (IS_SHADOW) {
  fs.writeFileSync(path.join(distChrome, "background.js"), bg, "utf8");
  fs.writeFileSync(path.join(distChrome, "popup-config.js"), popupConfig, "utf8");
  const compiledRoot = path.join(ROOT, ".trace-build");
  const compiledCore = path.join(compiledRoot, "extension-core");
  const compiledShadow = path.join(compiledRoot, "extension-shadow");
  if (!fs.existsSync(compiledCore) || !fs.existsSync(compiledShadow)) {
    throw new Error("Shadow build requires compiled extension core and observer outputs.");
  }
  copyDirFiltered(compiledCore, path.join(distChrome, "extension-core"));
  copyDirFiltered(compiledShadow, path.join(distChrome, "extension-shadow"));
  const shadowConfigPath = path.join(distChrome, "extension-shadow", "config.mjs");
  fs.writeFileSync(
    shadowConfigPath,
    fs.readFileSync(shadowConfigPath, "utf8").replace(/__TRACE_API_BASE__/g, TRACE_API_BASE),
    "utf8",
  );
  fs.writeFileSync(
    path.join(distChrome, "shadow-background.mjs"),
    'import "./extension-shadow/index.mjs";\nimport "./background.js";\n',
    "utf8",
  );
  fs.writeFileSync(
    path.join(distChrome, "shadow-control.html"),
    "<!doctype html><meta charset=\"utf-8\"><title>Trace shadow control</title>\n",
    "utf8",
  );
  chromeManifest = {
    ...packagedManifest,
...
if (IS_SHADOW) {
  fs.writeFileSync(path.join(distFf, "background.js"), bg, "utf8");
  fs.writeFileSync(path.join(distFf, "popup-config.js"), popupConfig, "utf8");
}
...
if (!IS_SHADOW) {
  ...
  console.log("Kept tracked Shared and Apple generated resources unchanged");
}

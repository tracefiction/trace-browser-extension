...
const earnedPermissionRuntimeConfig = {
  version: 3,
  registrationMode: IOS_EARNED_PERMISSION_ONBOARDING ? "static" : "dynamic",
...
// Safari derives Website Access from the complete permission-bearing manifest
// surface, including API permissions, host permissions, and static content
// script matches. The production earned-permission package therefore preserves
// the 0.6.3 declarations exactly. On a fresh or partial grant the restored
// static scripts are behavior-gated until the worker confirms the complete
// AO3/FFN bundle; the popup can request withheld required hosts from its direct
// user gesture without changing the upgrade contract.
...
const earnedCompatibilityContentScripts = [
  {
    ...archiveContentScript,
    js: [
      "popup-config.js",
...
manifest.content_scripts = SESSION_MODE === "disabled" || IOS_ACTIVE_TAB_PROBE
  ? []
  : IOS_EARNED_PERMISSION_ONBOARDING
    ? earnedCompatibilityContentScripts
